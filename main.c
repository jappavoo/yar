...
"-----------------------\n\n"	  
  ...
  "    failure (default %f)\n"
  " -f directory that the yar control synthetic filesystem mount point will be\n"
  "    created in.  The mount point name will be the pid of the yar instance\n"
  "    suffixed with .fs"
  ...
  "    be created in.  The link's name is process id (pid) of yar '.mon'.\n"
  ...
  " -K do not delete the log file on exit rather keep it -- useful for \n"
  "    debugging\n"
  " -L <directory path> if specified all debugging and error messages will be\n"
  "    sent to a log file name as the procsess id of yar suffixed with '.log'\n"
  "    This file will be placed in the directory specified\n\n"
...
monInit(bool initdir, char *dir, bool iszeroed)
{
  ...
  if (initdir) {
    ...
    if (!ttyInit(&(GBLS.mon.tty), tmp, NULL, NULL, NULL, NULL, true)) EEXIT();
  } else {
    if (!ttyInit(&(GBLS.mon.tty), NULL, NULL, NULL, NULL, NULL, true)) EEXIT();
  }
}
...
GBLSInit()
{
  ...
  ttyInit(&(GBLS.bcsttty),NULL,NULL,NULL,NULL,NULL,true);
...
int main(int argc, char **argv)
{
  ...
  }
  {
    char tmp1[PATH_MAX];
    char tmp2[PATH_MAX];
    snprintf(tmp1, sizeof(tmp1), "%s.mon", GBLS.bcstttylink);
    snprintf(tmp2, sizeof(tmp2), "%s.fs", GBLS.bcstttylink);
    if (!ttyInit(&GBLS.bcsttty, GBLS.bcstttylink,
		 tmp1, GBLS.mon.tty.link, tmp2, GBLS.fs.mntpt,
		 true)) EEXIT();
  }
